    ...
    <x-card maxWidth="600px">
        <form action="{{ route('admin.vaccines.store') }}" method="POST">
            ...
            <div style="margin-bottom: 1rem;">
                ...
                @error('vaccinator_username') <span style="color: var(--danger); font-size: 0.875rem;">{{ $message }}</span>
                @enderror
            </div>

            <x-button type="submit" fullWidth style="margin-top: 1.5rem; background-color: orange; border-color: orange;">
                Registrar Aplicação
            </x-button>
        </form>
    </x-card>
